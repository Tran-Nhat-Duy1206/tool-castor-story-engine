...
describe("Issue 1 — isNewLayoutBook detection", () => {
  ...
  beforeEach(async () => {
    bookDir = await mkdtemp(join(tmpdir(), "castor-layout-detect-"));
  ...
  it("returns false when story/ dir does not exist at all", async () => {
    const emptyDir = await mkdtemp(join(tmpdir(), "castor-empty-"));
    try {
...
describe("Issue 1 — write_truth_file per-book shim detection", () => {
  ...
  beforeEach(async () => {
    bookDir = await mkdtemp(join(tmpdir(), "castor-wt-shim-"));
    await mkdir(join(bookDir, "story"), { recursive: true });
  });
...
describe("Issue 2 — per-chapter promotion persists to pending_hooks.md", () => {
  ...
  beforeEach(async () => {
    bookDir = await mkdtemp(join(tmpdir(), "castor-perchapt-promo-"));
    await mkdir(join(bookDir, "story"), { recursive: true });
  });
