 ...
 *
 * CASTOR_* is authoritative; known legacy castor_* keys are read as a
...
const CLEAN_ENV = Object.fromEntries(
  Object.entries(process.env).filter(([k]) => !k.startsWith("castor_") && !k.startsWith("CASTOR_")),
...
describe("env compatibility resolver", () => {
  ...
  it("only mapped legacy castor_* set → compatibility fallback", () => {
    const env = { ...CLEAN_ENV, castor_STUDIO_PORT: "4601" };
    expect(castorEnv("CASTOR_STUDIO_PORT", env)).toBe("4601");
  });

  it("both set with same value → Castor value, no warning", () => {
    const env = { ...CLEAN_ENV, CASTOR_STUDIO_PORT: "4602", castor_STUDIO_PORT: "4602" };
  ...
  it("both set with different values → Castor value + non-secret key-name warning", () => {
    const env = { ...CLEAN_ENV, CASTOR_STUDIO_PORT: "4603", castor_STUDIO_PORT: "4604" };
    const warnings: string[] = [];
    expect(castorEnv("CASTOR_STUDIO_PORT", env, warnings)).toBe("4603");
    expect(warnings.join("\n")).toContain("castor_STUDIO_PORT");
    expect(warnings.join("\n")).toContain("deprecated");
  });

  it("unknown castor_* names are not magically mapped", () => {
    const env = { ...CLEAN_ENV, castor_TOTALLY_UNKNOWN_VAR: "x" };
...
describe("env layer normalization (llm-env)", () => {
  it("legacy castor_LLM_* in project .env is visible under CASTOR_LLM_* keys", async () => {
    const root = await tempRoot();
    await writeFile(join(root, ".env"), "castor_LLM_MODEL=legacy-model\ncastor_LLM_API_KEY=sk-legacy\n", "utf-8");
    ...
    expect(layers.project["castor_LLM_MODEL"]).toBeUndefined();
  });
  ...
  it("both files define the same key with different values → Castor key kept, warning emitted", async () => {
    ...
    await writeFile(globalEnvPath, "castor_LLM_MODEL=legacy-model\n", "utf-8");
    ...
    const layers = await loadLLMEnvLayers(
      root,
      { ...CLEAN_ENV, castor_LLM_MODEL: "legacy-process", CASTOR_LLM_MODEL: "castor-process" } as NodeJS.ProcessEnv,
    );
    expect(layers.process["CASTOR_LLM_MODEL"]).toBe("castor-process");
    expect(layers.process["castor_LLM_MODEL"]).toBeUndefined();
    expect((layers.warnings ?? []).join("\n")).toContain("castor_LLM_MODEL");
  });
  ...
  it("layer warnings surface without echoing values", async () => {
    ...
    expect(all).toContain("castor_LLM_API_KEY");
