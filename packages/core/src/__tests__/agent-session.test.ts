...
describe("runAgentSession cache — bookId switch", () => {
  ...
  beforeEach(async () => {
    projectRoot = await mkdtemp(join(tmpdir(), "castor-agent-cache-"));
  ...
  it("guards pi-agent stream context before calling streamSimple", async () => {
    ...
    expect(result.errorMessage).toContain("Castor context window guard");
    ...
    expect(events.some(
      (event: any) =>
        ...
        event.error.includes("Castor context window guard"),
    )).toBe(true);
  });
  ...
  it("keeps cached Agents isolated by projectRoot for the same sessionId", async () => {
    const model = { provider: "x", id: "y", api: "anthropic-messages" } as any;
    const pipeline = {} as any;
    otherProjectRoot = await mkdtemp(join(tmpdir(), "castor-agent-cache-other-"));
