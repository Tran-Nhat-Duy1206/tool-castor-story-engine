...
function buildChatPrompt(): string {
  return `You are the Castor general chat assistant.
...
function appendSkillGuidance(
  ...
  const skillLines = skills.usedSkills.flatMap((skill) => {
    const line = `- ${skill.id} (forced): ${skill.description}`;
...
function buildBookCreatePrompt(confirmed: boolean): string {
  if (!confirmed) {
    return `You are the Castor book creation assistant. This surface stages a long-form / serialized book draft and asks for confirmation before creation.
...
${commonOutputRules()}`;
  ...
  return `You are the Castor book creation assistant. The user has confirmed long-form / serialized book creation.
...
function buildShortPrompt(confirmedIntent?: "short_run" | "generate_cover"): string {
  if (confirmedIntent === "short_run") {
    return `You are the Castor Short assistant. The user has confirmed standalone short-fiction generation.
...
${commonOutputRules()}`;
  ...
  if (confirmedIntent === "generate_cover") {
    return `You are the Castor Short cover assistant. The user has confirmed cover generation or regeneration.
...
${commonOutputRules()}`;
  ...
  return `You are the Castor Short assistant. This surface clarifies standalone short-fiction or cover requests and asks for confirmation before production.
...
function buildScriptPrompt(confirmed: boolean): string {
  if (confirmed) {
    return `You are the Castor script creation assistant. The user has confirmed script creation.
...
${commonOutputRules()}`;
  ...
  return `You are the Castor script creation assistant. This surface turns a novel, idea, outline, or existing text into an editable script.
...
function buildStoryboardPrompt(confirmed: boolean): string {
  if (confirmed) {
    return `You are the Castor storyboard creation assistant. The user has confirmed storyboard creation.
...
${commonOutputRules()}`;
  ...
  return `You are the Castor storyboard creation assistant. This surface turns scripts, novel excerpts, ideas, or scene lists into editable storyboard tables and image prompts.
...
function buildInteractiveFilmPrompt(confirmed: boolean): string {
  if (confirmed) {
    return `You are the Castor interactive-film creation assistant. The user has confirmed interactive-film creation.
...
${commonOutputRules()}`;
  ...
  return `You are the Castor interactive-film creation assistant. This surface turns ideas, novels, scripts, outlines, or submission requirements into editable interactive film/game-script deliverables.
...
function buildInteractiveFilmAuthoringPrompt(projectId: string): string {
  return `You are the Castor interactive-film authoring guide for project "${projectId}".
...
function buildPlayPrompt(confirmedStart: boolean, playWorldExists: boolean): string {
  if (confirmedStart) {
    return `You are the Castor Play assistant. The user has confirmed starting an interactive world.
...
${commonOutputRules()}`;
  ...
  if (!playWorldExists) {
    return `You are the Castor Play assistant. This surface can start a new interactive world, but no world exists yet.
...
${commonOutputRules()}`;
  ...
  return `You are the Castor Play assistant. This surface only runs interactive worlds.
...
function buildEditPrompt(bookId: string | null): string {
  const name = bookId ?? "";
  return `You are the Castor external editing assistant. This surface only handles explicit content edits.
...
function buildBookPrompt(bookId: string): string {
  return `You are the Castor writing assistant, working on book "${bookId}".
...
export function buildAgentSystemPrompt(
  ...
  if (sessionKind === "book-create") return withSkills(buildBookCreatePrompt(isConfirmedAction(options, "create_book")));
  if (sessionKind === "short") {
    const confirmedIntent = isConfirmedAction(options, "short_run")
      ...
      : isConfirmedAction(options, "generate_cover")
        ? "generate_cover"
        : undefined;
    return withSkills(buildShortPrompt(confirmedIntent));
  }
  if (sessionKind === "play") return withSkills(buildPlayPrompt(isConfirmedAction(options, "play_start"), options.playWorldExists === true));
  if (sessionKind === "script") return withSkills(buildScriptPrompt(isConfirmedAction(options, "script_create")));
  if (sessionKind === "storyboard") return withSkills(buildStoryboardPrompt(isConfirmedAction(options, "storyboard_create")));
  if (sessionKind === "interactive-film") return withSkills(buildInteractiveFilmPrompt(isConfirmedAction(options, "interactive_film_create")));
  if (sessionKind === "interactive-film-authoring" && bookId) return withSkills(buildInteractiveFilmAuthoringPrompt(bookId));
  if (sessionKind === "edit") return withSkills(buildEditPrompt(bookId));
  if (sessionKind === "book" && bookId) return withSkills(buildBookPrompt(bookId));
  return withSkills(buildChatPrompt());
}
