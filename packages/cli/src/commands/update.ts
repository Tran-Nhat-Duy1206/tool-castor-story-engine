...
export const updateCommand = new Command("update")
  .description("Update Castor to the latest version")
  .action(async () => {
    try {
      ...
      const remoteVersion = execSync("npm view @actalk/castor version", {
        encoding: "utf-8",
      }).trim();
      ...
      execSync("npm install -g @actalk/castor@latest", { stdio: "inherit" });
      log(`Updated to ${remoteVersion}.`);
    } catch (e) {
      logError(`Update failed: ${e}`);
      log("You can also update manually: npm install -g @actalk/castor@latest");
      process.exit(1);
    }
  });
