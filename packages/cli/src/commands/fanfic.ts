...
import { deriveBookIdFromTitle, normalizePlatformOrOther, PipelineRunner, type BookConfig, type FanficMode } from "@actalk/castor-core";
  ...
  .action(async (opts) => {
    try {
      ...
      if (opts.json) {
        ...
        log("");
        log(`Next: castor write next ${bookId}`);
      }
    } catch (e) {
      if (opts.json) {
