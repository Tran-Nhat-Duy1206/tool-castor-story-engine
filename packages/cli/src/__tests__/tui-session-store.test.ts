...
describe("tui session store", () => {
  beforeAll(async () => {
    projectRoot = await mkdtemp(join(tmpdir(), "castor-tui-session-"));
  ...
  it("falls back to the only book in the project", async () => {
    ...
    const singleRoot = await mkdtemp(join(tmpdir(), "castor-tui-single-"));
  ...
  it("returns undefined when multiple books exist and no valid active binding is stored", async () => {
    const multiRoot = await mkdtemp(join(tmpdir(), "castor-tui-multi-"));
