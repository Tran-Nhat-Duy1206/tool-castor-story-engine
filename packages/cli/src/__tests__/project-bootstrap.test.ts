...
describe("project bootstrap", () => {
  ...
  beforeEach(async () => {
    tempDir = await mkdtemp(join(tmpdir(), "castor-bootstrap-"));
  ...
  it("creates a minimal Studio-first project when none exists", async () => {
    ...
    expect(config.name).toMatch(/^castor-bootstrap-/);
  ...
  it("returns false when the directory is already an Castor project", async () => {
