...
import { visibleChoices, applyEffects, initVarState, type VarState } from "@actalk/castor-core/interactive-film/evaluator";
import type { StoryGraph, Choice } from "@actalk/castor-core/interactive-film/graph-schema";
...
export function StoryPlayer({
  ...
  if (!node) return <div className="text-destructive">Thiếu nút: {currentId}</div>;
      ...
      {isEnding ? (
        <div className="border rounded-xl p-8 text-center space-y-4" data-testid="player-ending">
          ...
          <div className={c.muted} data-testid="player-unlocked">
            Kết đã mở khóa {unlocked.length} / {graph.endings.length}
          </div>
        ...
        <div className="space-y-2" data-testid="player-choices">
          ...
          {choices.length === 0 && <div className="text-destructive" data-testid="player-deadend">Đường này cụt</div>}
        </div>
      )}
