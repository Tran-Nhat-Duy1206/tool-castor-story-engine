...
export function BookDetail({
  ...
  const handleRewrite = async (chapterNum: number) => {
    const brief = window.prompt(
      data?.book.language === "en"
        ...
        : "Tùy chọn: nhập ý tưởng bổ sung cho lần viết lại này. Để trống sẽ giữ nguyên focus hiện tại.",
      "",
    );
  ...
  const handleRevise = async (chapterNum: number, mode: ReviseMode) => {
    const brief = window.prompt(
      data?.book.language === "en"
        ...
        : "Tùy chọn: nhập ý tưởng bổ sung cho lần chỉnh sửa này. Để trống sẽ giữ nguyên focus hiện tại.",
      "",
    );
    if (brief === null) return;
  ...
  const handleSync = async (chapterNum: number) => {
    const brief = window.prompt(
      data?.book.language === "en"
        ...
        : "Tùy chọn: nhập ghi chú bổ sung cho lần đồng bộ này. Để trống sẽ đồng bộ trực tiếp theo phần thân.",
      "",
    );
    if (brief === null) return;
  ...
  const handleConsolidate = async () => {
    await runBookAction("consolidate", async () => {
      ...
      return data?.book.language === "en"
        ...
        : `Đã hợp nhất ${result.archivedVolumes ?? 0} bản tóm tắt tập, giữ lại ${result.retainedChapters ?? 0} bản tóm tắt chương gần nhất.`;
    });
  };
  ...
  const handleReviseFoundation = async () => {
    const feedback = window.prompt(
      data?.book.language === "en"
        ...
        : "Nhập phản hồi để sửa lại nền tảng. Thao tác này sẽ viết lại nền tảng, không sửa trực tiếp phần thân.",
      "",
    );
    ...
    await runBookAction("revise-foundation", async () => {
      ...
      return "Nền tảng đã được sửa lại.";
    });
  };
  ...
  const handlePlan = async () => {
    const context = window.prompt(
      data?.book.language === "en"
        ...
        : "Tùy chọn: ghi chú bổ sung cho việc lập kế hoạch chương tiếp theo.",
      "",
    );
  ...
  const handleCompose = async () => {
    const context = window.prompt(
      data?.book.language === "en"
        ...
        : "Tùy chọn: ghi chú bổ sung cho việc lắp ghép chương tiếp theo.",
      "",
    );
    if (context === null) return;
    await runBookAction("compose", async () => {
      ...
      return data?.book.language === "en"
        ...
        : `Đã lắp ghép chương ${result.chapterNumber ?? "?"}: ${result.title ?? ""}`;
    });
  };
  ...
  const handleRepairState = async (chapterNum: number) => {
    await runBookAction(`repair-state-${chapterNum}`, async () => {
      ...
      return `Đã sửa trạng thái cho chương ${chapterNum}.`;
    });
  };
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button
            ...
            title={reviewMode === "manual"
              ? "Soát thủ công: dừng ngay sau khi viết xong, bạn tự nhấp Duyệt/Chỉnh sửa/Thông qua (nhanh hơn, kiểm soát tốt hơn). Nhấp để chuyển về tự động."
              : "Soát tự động: tự động soát lỗi và viết lại khi cần sau khi viết xong (nhàn hơn nhưng chậm hơn). Nhấp để chuyển sang thủ công — dừng ngay khi viết xong."}
          ...
          >
            ...
            {reviewMode === "manual" ? "Soát: thủ công · dừng khi viết xong" : "Soát: tự động"}
          </button>
          <button
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex gap-1.5 justify-end opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <button
                        ...
                        className="p-2 rounded-lg bg-secondary text-muted-foreground hover:text-primary hover:bg-primary/10 transition-all shadow-sm disabled:opacity-50"
                        title="Đồng bộ truth/state từ chương đã chỉnh sửa"
                      >
