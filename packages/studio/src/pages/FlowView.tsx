...
export default function FlowView({
  ...
  const onConnect = async (conn: { source: string | null; target: string | null }) => {
    ...
    if (!src) return;
    await post(addChoiceDelta(src, { id: genChoiceId(), text: tr("Lựa chọn mới", "New choice"), targetNodeId: conn.target }));
  };
  ...
  return (
    <div className="flex flex-col h-full p-5 gap-3" data-testid="flow-view">
      <div className="flex items-center gap-3 text-sm shrink-0">
        ...
        >
          {editing ? tr("Xong chỉnh sửa", "Done editing") : tr("Chỉnh sửa", "Edit")}
        </button>
        {editing && (
          <button
      ...
      {stats && (
        ...
        >
          <span>{tr("Tổng nút", "Nodes")} {stats.total}</span>
          <span>{tr("Nhánh", "Branches")} {stats.branch}</span>
          <span>{tr("Kết", "Endings")} {stats.ending}</span>
          <span>{tr("Đường cụt", "Dead ends")} {stats.deadEnd}</span>
