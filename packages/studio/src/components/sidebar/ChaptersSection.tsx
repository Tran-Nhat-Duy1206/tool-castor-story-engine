...
export function ChaptersSection({ bookId, isVi }: ChaptersSectionProps) {
  ...
  return (
    <SidebarCard title={isVi ? "Chương" : "Chapters"}>
      {chapters.length === 0 ? (
        <p className="text-[15px] leading-6 text-muted-foreground/50 italic">
          {isVi ? "Chưa có chương" : "No chapters"}
        </p>
      ) : (
            ...
            return (
              <li
                ...
                <span className="truncate flex-1">
                  {String(ch.number).padStart(2, "0")} {ch.title || `Chương ${ch.number}`}
                </span>
