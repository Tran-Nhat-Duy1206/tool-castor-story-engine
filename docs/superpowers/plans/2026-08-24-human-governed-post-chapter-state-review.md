...
export const ResolvedReviewReceiptSchema = z.object({
  ...
  evidence: z.array(ReceiptEvidenceEntrySchema),   // REQUIRED fourth layer (spec §7/§23): {itemId, evidence} item-aligned; [] valid
...
**Task 7 fix-up amendment (post-review, commit-scoped to Task 7 semantics only):**
- If the audit/review cycle CHANGES the publishable prose, the FINAL prose must be re-settled through the existing RuntimeStateDelta-producing settlement path (`WriterAgent.settleChapterState`) before publication; the proposal must be built from that final delta and anchored to the final durable bytes. The pre-revision delta is never reused.
- A governed publishable (passed-audit) chapter whose RuntimeStateDelta is missing or schema-invalid FAILS CLOSED (throw before any authoritative write). It must NOT fall back to legacy immediate state application, `ready-for-review`, snapshots, or fact-history sync. Governance is never inferred from data-shape validity alone; the runner resolves an explicit validated proposal for every governed publication.
- `needs-state-review` is scheduler/batch PUBLICATION SUCCESS but an ADVANCEMENT STOP: the scheduler counts it as a produced chapter, never retries it as a failure, and pauses further generation until human State Review completes. Legacy `ready-for-review` scheduler-success behavior is preserved for non-governed flows.
...
Check order (§9.A): loaded-artifact status must be `active` (shell/stale ⇒ respective errors) → `expectedReviewRevision` ⇒ `edit_conflict` → `proseRevision` vs recomputed file hash ⇒ `state_review_stale` → `baseCanonRevision` vs `readStoryCanon` ⇒ `state_review_conflict` → temporal rules §20 against `params.durableHead` (normal source===effective===head+1; correction source<head ⇒ effective=head+1; head moved past effective ⇒ APPLY-ZERO error) → completeness (AI `undecided` ⇒ `state_review_incomplete`; invalid user item ⇒ `state_review_invalid_change`+itemId) → compile ONE `RuntimeStateDelta` (delta.chapter = effectiveChapter) from `resolveReviewItemEffectiveChange` per item → `buildRuntimeStateArtifactsFromSnapshot({snapshot: readLiveRuntimeStateSnapshot(), delta, language})` → receipt (typed layers: proposals/decisions/effectiveChanges + REQUIRED item-aligned `evidence` entries from every item carrying `ReviewEvidenceSchema` metadata — spec §7/§23; `[]` when none) → index update in memory. Zero-effective-change (amended — the earlier "canon/projection writes EMPTY" shorthand froze applied-progression bookkeeping and contradicted reducer/settlement behavior): PREPARE still compiles an op-less RuntimeStateDelta at `effectiveChapter` and runs it through the SAME pure reducer path so `manifest.lastAppliedChapter`/`currentState.chapter` ADVANCE through the confirmed temporal slot; candidate canon/projections/snapshot are built from that advanced state; `zeroEffectiveChange` and receipt `confirmed-no-changes` remain SEMANTIC classifications only. Historical chapter summaries (amended): proposals are SOURCE-chaptered (`row.chapter === sourceChapter`, validated; mismatch ⇒ `invalid_change`+itemId); application retargets the row to `effectiveChapter` for the compiled delta while the original proposal stays unchanged in history, and `receipt.effectiveChanges` records the APPLIED (retargeted) representation per design §23.
...
Amendment (integration glue found by the primary E2E): `state/state-bootstrap.ts` unconditionally rewrote `manifest.lastAppliedChapter := contiguous chapter-file prefix` on EVERY load, silently clamping a confirmed forward-applied head (historical correction effective = confirmedHead+1 with no future prose) back to the prefix and leaving `current_state.chapter` ahead ⇒ durable Canon inconsistency (`current_state_ahead_of_manifest`) for every later reader. Fix: bootstrap now honors a structured head ABOVE the prefix only when the structured documents AGREE (`current_state.chapter === manifest.lastAppliedChapter` — exactly the post-Final-Confirm shape Task 12 atomically writes); hallucinated markdown numbers alone can still never manufacture a head, preserving the original anti-hallucination rule. Earlier advance-head unit tests missed this because their fixtures seeded chapter prose up to the effective slot (prefix == head), so normalization never fired.
Amendment 2 (follow-up fix — PROSE SOURCE ≠ SEMANTIC SLOT): making the forward head durable exposed the writer/publication seam, which still assumed `head == prefix`. (1) Governed/deferred Writer settlement (`deferStateApplication`, Task 6/7 contract) no longer builds candidate runtime artifacts against the LIVE snapshot during generation — the source-oriented proposal delta rides the result unapplied; legacy non-deferred behavior is byte-identical. (2) Governed publication now derives `effectiveChapter` from the CONFIRMED SEMANTIC head via ONE shared §20 helper (`resolveEffectiveChapter`) used by Task 10 rebuild, Task 11 confirm revalidation, and pipeline publication alike — so healthy books publish source26/effective26 while post-correction books publish source26/effective27 with NO manual rebuild round-trip; stacked corrections keep working with no `head ≤ prefix+1` bound. (3) The next prose chapter number is still prefix+1 (no skipped prose); Task 11/12 apply confirmed semantics at the effective slot and write snapshots/<effective>, leaving historical snapshots byte-identical. No reducer reapply mode, no bootstrap change beyond the reviewed fix. The primary historical E2E now drives the PUBLIC Task 10 boundary (`PipelineRunner.regenerateStateReview` via `deps.createWriter`).
Amendment 3 (closure review): the deferred contract is extended to EVERY governed-reachable settlement/persistence branch — audit-revised final-prose re-settlement (`deferStateApplication: true`), state-validation recovery retries (flag threaded through `validateChapterTruthPersistence` → `retrySettlementAfterValidationFailure`; legacy repair/sync flows keep baseline semantics by omission), and non-publishable outcomes (audit-failed/degraded persistence passes `deferStateApplication` whenever the output carries a proposal delta, so no legacy rebuild can live-apply source semantics against the confirmed head). The real runner→writer flag propagation is pinned in the future-progression suite; two pipeline-runner tests that accidentally relied on legacy audit-failure live application were converted to pin the governed proposal-only contract.
